...
//lexical cast for reportss_bounds_probes_NUM_PROBES
#include "../ext/boost/dynamic_bitset.hpp"
#include <boost/lexical_cast.hpp>
#include <stdlib.h>


SSMean::SSMean(const Options &opts) : SearchEngine(opts), current_state(g_initial_state()) {	
	...
	//this->RanGen3 = new CRandomMersenne((unsigned)time(NULL));
	g_rng.seed(2015);
}
				...
				for (it = L.begin(); it != L.end(); it++) {	
					...
					map<Type, SSNode>::iterator queueIt = queue.find( t );
			   		if( queueIt != queue.end() )
			   		{
						...
						//int rand_100 =  RanGen3->IRandom(0, 99);  //(int)g_rng.next(100);
                          	 		int rand_100 = g_rng() * 100;
...
void SSMean::BFS(SSNode root, Type type) {
	//std::queue<SSNode> D;
	std::deque<SSQueue> D;
	SSQueue s1;
	s1.setNode(root);
...
void SSMean::generateGeneratedReport() {
...
void SSMean::generateExpandedReport() {
...
double SSMean::getProbingResult() {
        double expansions = 0;
...
static SearchEngine *_parse(OptionParser &parser) {
	...
	SSMean *engine = 0;
	if (!parser.dry_run()) {
		engine = new SSMean(opts);
	}
	return engine;
}


static Plugin<SearchEngine> _plugin("ss_mean", _parse);
