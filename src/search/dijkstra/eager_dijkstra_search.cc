...
int EagerDijkstraSearch::returnMaxF(vector<int> levels) {
	int max = levels.at(0);
...
int EagerDijkstraSearch::returnMinF(vector<int> levels) {
	int min = levels.at(0);
	for (size_t i = 0; i < levels.size(); i++) {
		if (min > levels.at(i)) {
			min = levels.at(i);
...
void EagerDijkstraSearch::generateExpandedReport() {
	...
	string directory = "mkdir /home/marvin/marvin/dijkstra/"+heuristica+"/reportdijkstra/"+dominio+"/";
	if (system(directory.c_str())) {
		cout<<"Directory created successfully!"<<endl;
	}

	string nBL = "/home/marvin/marvin/dijkstra/"+heuristica+"/reportdijkstra/"+dominio+"/"+tarefa;
