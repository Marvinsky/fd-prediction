...
void EagerSearchMax::initialize() {
    if(use_saved_pdbs){
...
void EagerSearchMax::statistics() const {
    search_progress.print_statistics();
    search_space.statistics();
}

SearchStatus EagerSearchMax::step() {
...
void EagerSearchMax::generateSSCCReport() {
...
pair<SearchNode, bool> EagerSearchMax::fetch_next_node() {
...
void EagerSearchMax::reward_progress() {
    // Boost the "preferred operator" open lists somewhat whenever
    // one of the heuristics finds a state with a new best h value.
    open_list->boost_preferred();
}

void EagerSearchMax::dump_search_space() {
    search_space.dump();
}

void EagerSearchMax::update_jump_statistic(const SearchNode &node) {
...
void EagerSearchMax::print_heuristic_values(const vector<int> &values) const {
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    EagerSearchMax *engine = 0;
    if (!parser.dry_run()) {
        opts.set<bool>("mpd", false);
        engine = new EagerSearchMax(opts);
    }
...
static Plugin<SearchEngine> _plugin_astar("astar_max", _parse_astar);
