...
void read_everything(istream &in) {
    ...
    cout<<"read_and_verify_version finished"<<endl;
    read_metric(in);
    cout<<"read_metric finished"<<endl;
    read_variables(in);
    cout<<"read_variables finished"<<endl;
    read_mutexes(in);
    cout<<"read_mutexes finished"<<endl;
    g_initial_state_data.resize(g_variable_domain.size());
    check_magic(in, "begin_state");
    for (size_t i = 0; i < g_variable_domain.size(); ++i) {
        in >> g_initial_state_data[i];
    }
    cout<<"initial state read"<<endl;
...
double pdb_gen_time_limit = 1000;
