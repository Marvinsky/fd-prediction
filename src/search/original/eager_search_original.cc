#include "eager_search_original.h"
...
void EagerSearchOriginal::initialize() {
...
void EagerSearchOriginal::statistics() const {
    search_progress.print_statistics();
    search_space.statistics();
}

SearchStatus EagerSearchOriginal::step() {
...
pair<SearchNode, bool> EagerSearchOriginal::fetch_next_node() {
...
void EagerSearchOriginal::dump_search_space() {
    search_space.dump();
}

void EagerSearchOriginal::update_jump_statistic(const SearchNode &node) {
    if (f_evaluator) {
...
void EagerSearchOriginal::print_heuristic_values(const vector<int> &values) const {
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    EagerSearchOriginal *engine = 0;
    if (!parser.dry_run()) {
        opts.set<bool>("mpd", false);
        engine = new EagerSearchOriginal(opts);
    }
...
static Plugin<SearchEngine> _plugin_astar("astar_original", _parse_astar);
